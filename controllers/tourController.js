...
exports.isIdExist = (req, res, next, val) => {
  ...
  if (!idList.includes(val * 1)) {
    ...
    //The idea here is after we return next function will not be called so no more middlewares after this onee.
...
exports.checkBody = (req, res, next) => {
  if (!req.body.name || !req.body.price) {
    return res.status(400).json({
      status: 'fail',
      message: 'The body must contain both name and price',
    });
  }
  next();
};
